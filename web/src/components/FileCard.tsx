...
//         setDownloading(false)
//         setDownloadSuccess(true)
//         addNotification("Download started", "info") // ✅ Notify
//         setTimeout(() => setDownloadSuccess(false), 1500)
//       } catch (err: any) {
//         setDownloading(false)
...
//         await moveToTrash(id)
//         if (onTrashed) onTrashed()
//         addNotification(`"${fileName}" moved to trash`, "error") // ✅ Notify
//       } catch (err: any) {
//         addNotification("Failed to move file to trash", "error")
//       }
//       break
...
//         setStarred(updated.isStarred)
//         addNotification(
//           updated.isStarred ? `"${fileName}" starred` : `"${fileName}" unstarred`,
//           "success"
//         )
//         if (onStarToggled) onStarToggled(updated.isStarred)
//       } catch (err: any) {
//         addNotification("Failed to toggle star", "error")
//       }
//       break
...
//     try {
//       await renameItem(id, newName); // ✅ send to backend
//       setFileName(newName); // update UI
//       addNotification(`Renamed to "${newName}"`, "success");
//     } catch (err: any) {
...
import FilePreview from "../components/FilePreview"
import { moveToTrash, getFileUrl, downloadFile, renameItem, toggleStar } from "../lib/item.service"
import { useToast } from "../context/ToastContext" // ✅ new toast system
    ...
    switch (action) {
      ...
      case "delete":
        try {
          ...
          addToast({
            ...
            description: `"${fileName}" was moved to trash`,
          })
        } catch (err: any) {
          addToast({
            type: "error",
            title: "Delete Failed",
            description: err.message || "Could not move file to trash",
          })
        }
        break
      ...
      case "star":
        try {
          ...
          addToast({
            type: "success",
            title: updated.isStarred ? "File Starred" : "File Unstarred",
            description: `"${fileName}" ${updated.isStarred ? "starred" : "unstarred"}`,
          })
          if (onStarToggled) onStarToggled(updated.isStarred)
        } catch (err: any) {
          addToast({
            type: "error",
            title: "Star Toggle Failed",
            description: err.message || "Could not toggle star",
          })
        }
        break
    ...
    <>
      ...
      {canRename && (
        <RenameModal
          ...
          onRename={async (newName) => {
            try {
              await renameItem(id, newName) // ✅ backend call
              setFileName(newName)
              addToast({
                type: "success",
                title: "File Renamed",
