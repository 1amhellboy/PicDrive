...
const Trash: React.FC<TrashProps> = ({ viewMode }) => {
    ...
    try {
      ...
      if (pushEnabled()) {
        addToast({
          type: "success",
          title: "Item Restored",
          description: `"${name || "Item"}" was restored successfully.`,
        })
      }
    } catch (err: any) {
      if (pushEnabled()) {
    ...
    try {
      ...
      if (pushEnabled()) {
        addToast({
          type: "error",
          title: "Item Deleted",
          description: `"${name || "Item"}" was permanently deleted.`,
        })
      }
    } catch (err: any) {
      if (pushEnabled()) {
    ...
    try {
      ...
      if (pushEnabled()) {
        addToast({
          type: "info",
          title: "Trash Emptied",
          description: "All trashed items were permanently deleted.",
        })
      }
    } catch (err: any) {
      if (pushEnabled()) {
        addToast({
          ...
          description: err.message || "Could not empty trash",
        })
      }
    } finally {
      setEmptying(false)
  ...
  const TrashActions: React.FC<{ id: string; type: "file" | "folder"; name: string }> = ({
    id,
    type,
    name,
  }) => (
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md">
          ...
          {folders.map((folder) => (
            ...
            >
              ...
              <div className="w-24 text-gray-500 dark:text-gray-400">—</div>
              <div className="w-40 flex justify-end">
                <TrashActions id={folder.id} type="folder" name={folder.name} />
              </div>
            </div>
          ))}
          ...
          {files.map((file) => (
            ...
            >
              ...
              <div className="w-40 flex justify-end">
                <TrashActions id={file.id} type="file" name={file.name} />
              </div>
            </div>
          ))}
        </div>
